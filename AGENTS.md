...
- **`main.rs`** — Clap-derived CLI struct and orchestration. Flags: `--file` / `-f` (required, path to workflow YAML), `--provider` (advisory provider: `ghsa`, `osv`, or `all`; defaults to `all`), `--json` (structured JSON output), `--scan` (scan action repos for languages/ecosystems), `--github-token` (or `GITHUB_TOKEN` env var). Creates an `Auditor` and calls `auditor.audit()`.
